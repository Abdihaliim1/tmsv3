...
export const generateDriverSettlementPDF = (
  ...
  const COMPANY = getCompanyInfo(company);
  ...
  // Logo placeholder left (aligned with header block)
  ...
  [COMPANY.address, addr2, COMPANY.country].filter(Boolean).forEach((line) => {
  ...
  drawText(doc, 'Thank You For Your Business', margin + contentW / 2, pageHeight - margin - 0.50, {
    size: FONT_SIZES.body,
    bold: false,
    align: 'center',
  });
...
export const generateDispatcherSettlementPDF = (
  ...
  generateDriverSettlementPDF(settlement, dispatcher, loads, allSettlements, company);
};
